...
void generateRS(map<string, Definition>& grammar, string entry, vector<string>& ans){
    Definition d = grammar[entry];
    Production p = d.getRandomProduction();
    string word;
    for(vector<string>::iterator it = p.begin(); it != p.end(); ++it){
        word = *it;
        if (word[0]=='<') generateRS(grammar, word, ans);
...
int main(int argc, char *argv[])
{
  ...
  for (int i =1; i <= 3; i++){
       ...
       generateRS(grammar, "<start>", sentence);
