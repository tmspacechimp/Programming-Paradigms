...
bool bfs(string &source, string &target, imdb &db)
{
  ...
  {
    path tmp = partialPaths.front();
    partialPaths.pop_front();
    vector<film> credits;
    db.getCredits(tmp.getLastPlayer(), credits);
    for (unsigned int i = 0; i < credits.size(); i++)
    {
      if (previouslySeenFilms.find(credits[i]) == previouslySeenFilms.end())
      {
        previouslySeenFilms.insert(credits[i]);
        vector<string> cast;
        ...
        for (unsigned int j = 0; j < cast.size(); j++)
        {
          if (previouslySeenActors.find(cast[j]) == previouslySeenActors.end())
          {
            ...
            path entry = tmp;
            entry.addConnection(credits[i], cast[j]);
            if (cast[j] == target)
            {
              cout << entry << endl;
              return true;
