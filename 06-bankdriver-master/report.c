...
{
  sem_wait(&(bank->sem_transferLock));
  ...
  {
    sem_post(&(bank->sem_transferLock));
...
int Report_DoReport(Bank *bank, int workerNum)
{
  ...
  sem_wait(&(bank->sem_checkReport));
  bank->workerCntdwn--; 
  if(bank->workerCntdwn==0){
  ...
  {
    // We've run out of report storage for the bank
    bank->workerCntdwn=numWorkers;
    for (int i=0; i< numWorkers-1; i++){
      sem_post(&(bank->sem_newDay)); 
  }
    sem_post(&(bank->sem_checkReport));
    return -1;
  }
