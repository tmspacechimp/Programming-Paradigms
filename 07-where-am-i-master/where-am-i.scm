...
(define (it lst c) 
  (if (null? lst) '()
    (append (intersect c (car lst)) (it (cdr lst) c))
  )
)

(define (intersection-points lst)
  (if (null? lst) '()
    (append (it (cdr lst) (car lst)) (intersection-points (cdr lst)))
  
  )
)

(define (distance-product c l)

( apply * (map (lambda (circle) (if (equal? c circle ) 1 (dist c circle))) l))

)

(define (rate-points l)
  (map (lambda (c) (list (distance-product c l) c)) l)
...
(define (clumped-points l)
  (map (lambda (p) (car (cdr p))) 
  (prefix-of-list (sort-points(rate-points l)) (quotient (length l) 2)))
...
(define (average-point l)
  ( let ((avg
  (list (/ (apply + (map (lambda (co) (car co)) l)) (length l)) (/ (apply + (map (lambda (co) (car (cdr co))) l)) (length l)))))
...
(define (where-am-i d s)

(quicksort (map (lambda (g)(best-estimate g) )  (all-guesses d s)))
