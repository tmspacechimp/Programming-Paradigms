          ...
          int numWorkers)
{
  ...
  sem_init(&(bank->sem_newDay),0,0);
  sem_init(&(bank->sem_checkReport),0,1);
  sem_init(&(bank->sem_transferLock),0,1);
...
Bank_Balance(Bank *bank, AccountAmount *balance)
{
  ...
  for(unsigned int i=0; i<bank->numberBranches; i++){
    sem_wait(&(bank->branches[i].sem_branch));
  }
  ...
  *balance = bankTotal;
  for(unsigned int i=0; i<bank->numberBranches; i++){
    sem_post(&(bank->branches[i].sem_branch));
  }
  return 0;
}
